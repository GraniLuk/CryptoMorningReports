...
async def run_report(report_type="daily"):
    logging.info(f'{report_type.capitalize()} report function started at {datetime.now().isoformat()}')
    ...
    try:
        ...
        try:
            if report_type == "daily":
                ...
                await process_weekly_report(conn, telegram_enabled, telegram_token, telegram_chat_id)
        finally:
            conn.close()
...
def manual_trigger(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return func.HttpResponse(f"{report_type.capitalize()} report executed successfully", status_code=200)
    except Exception as e:
