...
def calculate_indicators(symbols: List[Symbol], conn, target_date: date = None) -> tuple[PrettyTable, PrettyTable]:
    ...
    for symbol in symbols:
        try:
            ...
            start_date = target_date - timedelta(days=400)  # Extra days for sufficient history
            end_date = target_date + timedelta(days=1)  # Add one day to include target_date
            ...
            if df.empty:
                app_logger.warning(f"No data available for {symbol.symbol_name} up to {target_date}")
                continue
