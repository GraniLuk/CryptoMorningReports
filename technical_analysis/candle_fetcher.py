...
class CandleFetcher:
    ...
    def fetch_candles(
        ...
        for symbol in symbols:
            ...
            if candle is not None:
                ...
                self.logger.warning(f"Failed to fetch {self.timeframe} candle for {symbol.symbol_name}")
    ...
    def check_if_all_candles(self, symbol, conn, days_back: int = 30):
        ...
        self.logger.info(f"Checking {self.timeframe} candles for {symbol.symbol_name} from {start_time} to {end_time}")
        
        # Get existing candles from DB
        all_candles = repo.get_candles(symbol, start_time, end_time)
        
        if not all_candles:
            self.logger.info(f"No {self.timeframe} candles found in DB for {symbol.symbol_name}, fetching all")
            current_time = start_time
            while current_time <= end_time:
                self.logger.debug(f"Fetching {self.timeframe} candle for {symbol.symbol_name} at {current_time}")
            ...
            self.logger.info(f"Found {len(all_candles)} {self.timeframe} candles in DB for {symbol.symbol_name}")
            ...
            if len(all_candles) > 1:
                ...
                for i in range(len(all_candles) - 1):
                    ...
                    if actual_diff > expected_diff:
                        self.logger.info(f"Found gap in {self.timeframe} candles for {symbol.symbol_name} between {current_candle.end_date} and {next_candle.end_date}")
                        # Fetch missing candles
                        current_time = current_candle.end_date + expected_diff
                        while current_time < next_candle.end_date:
                            self.logger.debug(f"Fetching missing {self.timeframe} candle for {symbol.symbol_name} at {current_time}")
                            self.fetch_function(symbol, current_time, conn)
